...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse<Order>> create(@RequestBody OrderCreateRequest request) {
        try {
            ...
            return ResponseEntity.ok().body(ApiResponse.SUCCESS(HttpStatus.OK.toString(), "Create order successfully", newOrder));
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse<Order>> read(@PathVariable Long orderId) {
        try {
            ...
            return ResponseEntity.ok().body(ApiResponse.SUCCESS(HttpStatus.OK.toString(), "Read order successfully", order));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.ERROR(HttpStatus.BAD_REQUEST.toString(), e.getMessage(), null));
        }
    }
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<Order>>> readAll(Integer pageNum, Integer pageSize) {
        List<Order> orders = orderService.readAll(pageNum, pageSize);

        if (orders.isEmpty()) {
            return ResponseEntity.ok().body(ApiResponse.SKIP_AS_GOOD(HttpStatus.OK.toString(), "No Order Exists", null));
        }

        return ResponseEntity.ok().body(ApiResponse.SUCCESS(HttpStatus.OK.toString(), "Get all orders successfully", orders));
    }

    @PutMapping("/{orderId}")
    public ResponseEntity<ApiResponse<Order>> update(@PathVariable Long orderId, @RequestBody OrderUpdateRequest request) {
        try {
            Order updated = orderService.update(orderId, request.toEntity());

            return ResponseEntity.ok().body(ApiResponse.SUCCESS(HttpStatus.OK.toString(), "Update order successfully", updated));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.ERROR(HttpStatus.BAD_REQUEST.toString(), e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> delete(@PathVariable Long orderId) {
        try {
            ...
            return ResponseEntity.ok().body(ApiResponse.SUCCESS(HttpStatus.OK.toString(), "Delete order successfully", null));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.ERROR(HttpStatus.BAD_REQUEST.toString(), e.getMessage(), null));
        }
    }
